...
/**
 * Konvertierung diverser Exceptions in eine halbwegs vernuenftige
 * Fehlermeldungen, falls die Exceptions nicht durch spezfische ExceptionMapper
 * abgebildet werden
 * 
 * Wesentlicher Code von:
 * http://securesoftwaredev.com/2013/08/19/how-to-implement
 * -input-validation-for-rest-resources
 * 
...
public class DefaultExceptionMapper implements ExceptionMapper<Throwable> {
	...
	public Response toResponse(Throwable exception) {
		...
		if (badRequestException != null) {
			return Response.status(BAD_REQUEST)
					.entity(badRequestException.getMessage()).build();
		}
	...
	private static ViolationReport toViolationReport(
		...
		for (ConstraintViolation<?> v : violations) {
			...
			final ResteasyConstraintViolation resteasyConstraintViolation = new ResteasyConstraintViolation(
					PARAMETER, path, message, value);
